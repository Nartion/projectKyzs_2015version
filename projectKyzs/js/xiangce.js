...
window.onload = function() {
    ...
    var oPage = document.getElementById('page').innerHTML;
    var imgs = oBox.getElementsByTagName("img");
    var imgURLs = [];
    for (var n = 0; n < imgs.length; n++) {
        imgURLs[n] = imgs[n].src;
    }
    var i = iNow = 0;
    for (i = 0; i < aLi.length; i++) {
        ...
        aLi[i].onclick = function() {
            ...
            oFrame.innerHTML = '<img src=' + imgURLs[this.index] + ' />';
            var oImg = oFrame.getElementsByTagName('img')[0];
            var iWidth = oImg.width;
            var iHeight = oImg.height;
            var wPer = iHeight/iWidth;
            var hPer = iWidth/iHeight;
            if (iWidth > 960) {
                iWidth = 960;
                iHeight = (iWidth * wPer);
            };
    ...
    aBli[0].onclick = function() {
        ...
        oFrame.innerHTML = '<img src=' + imgURLs[iNow-1] + ' width="100%" height="100%" />';
    };
    ...
    aBli[3].onclick = function() {
        startMove(oFrame, {
            opacity: 0,
            left: aImg[iNow - 1].offsetLeft,
            top: aImg[iNow - 1].offsetTop,
            width: 150,
            height: 100
        }, function() {
